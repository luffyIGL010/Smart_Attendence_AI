...
def student_dashboard():
    ...
    for i,sub_node in enumerate(subjects):
        ...
        def unenroll_button():
                if st.button('Unenroll this course',type='tertiary',width='stretch'):
...
def student_screen():
    ...
    st.header('Login using facial recognition',text_alignment="center")
    st.space()
    st.space()
    show_registration=False
    photo_source=st.camera_input("Take a selfie to mark your attendance")
    if photo_source:
        ...
        with st.spinner('AI is scanning'):
            ...
            if num_faces==0:
                ...
                if detected:
                    ...
                    student=next((s for s in get_all_students() if s['student_id']==student_id),None)
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name=st.text_input("Enter your name",placeholder='E.g. ABC')
            ...
            audio_data=None
            try:
                audio_data=st.audio_input("Record a short phrase like I am present,My name is Saurav")
            except Exception:
                    ...
                    with st.spinner('Creating profile....'):
                        ...
                        if encodings:
                            ...
                            if response_data:
                                ...
                                st.toast(f'New Student {new_name}')

                                time.sleep(1)
                                st.rerun()

                        else:
                            st.error('Couldnt capture your facical features for registration..')
                else:
